...
/// リマインダー設定。バックアップにも含めるので ShiftTechoCore で定義する。
...
public enum BackupCodec {
    ...
    public static func decode(_ data: Data) throws -> BackupDocument {
        ...
        guard document.assignments.allSatisfy({ ShiftTechoCalendar.date(fromDayKey: $0.dayKey) != nil }) else {
    ...
    /// 書き出しファイル名（例: shifttecho-backup-2026-08-30.json）。
    public static func fileName(exportedAt: Date, calendar: Calendar = ShiftTechoCalendar.japan) -> String {
        "shifttecho-backup-\(ShiftTechoCalendar.dayKey(for: exportedAt, calendar: calendar)).json"
    }
}
