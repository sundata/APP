        ...
        HStack(spacing: ShiftTechoTheme.Spacing.m) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(definition.name.isEmpty ? "名称未設定" : definition.name)
                    .font(ShiftTechoTheme.bodyFont.weight(.medium))
                Text(subtitle)
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("色") {
                    LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 4), spacing: ShiftTechoTheme.Spacing.s) {
    ...
    var body: some View {
        Picker(title, selection: $minute) {
            ForEach(Self.steps, id: \.self) { value in
                Text(ShiftTechoCalendar.timeText(minuteOfDay: value)).tag(value)
            }
        }
    }
}
