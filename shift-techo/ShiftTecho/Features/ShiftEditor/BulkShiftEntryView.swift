...
struct BulkShiftEntryView: View {
    ...
    init(environment: AppEnvironment, initialDayKey: String, onChange: @escaping (String) -> Void) {
        ...
        let start = ShiftTechoCalendar.date(fromDayKey: initialDayKey) ?? environment.clock.now
    ...
    private var dayKeys: [String] {
        ShiftTechoCalendar.dayKeys(from: startDate, to: endDate)
    }

    private var isOverLimit: Bool { dayKeys.count > ShiftTechoStore.bulkAssignmentLimit }
            ...
            Form {
                Section("期間") {
                    DatePicker("開始日", selection: $startDate, displayedComponents: .date)
                        .environment(\.calendar, ShiftTechoCalendar.japan)
                    ...
                    if isOverLimit {
                        Text("一度に登録できるのは \(ShiftTechoStore.bulkAssignmentLimit) 日までです。期間を短くしてください。")
