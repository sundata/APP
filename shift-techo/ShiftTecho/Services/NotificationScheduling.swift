...
struct UpcomingShift: Hashable, Sendable {
    ...
    var bodyText: String {
        guard let date = ShiftTechoCalendar.date(fromDayKey: dayKey) else { return name }
        let day = ShiftTechoCalendar.displayDate(for: date)
        guard let startMinute else { return "\(day) は \(name) です。" }
        return "\(day) は \(name)（\(ShiftTechoCalendar.timeText(minuteOfDay: startMinute)) 開始）です。"
    }
}

#if canImport(UserNotifications)
struct LocalNotificationScheduler: NotificationScheduling {
    static let monthlyIdentifier = "shifttecho.reminder.monthly"
    static let nextShiftPrefix = "shifttecho.reminder.nextShift."
    ...
    func scheduleMonthlyReminder(day: Int, minuteOfDay: Int) async {
        ...
        components.timeZone = ShiftTechoCalendar.japan.timeZone
    ...
    func scheduleNextShiftReminders(_ shifts: [UpcomingShift], minuteOfDay: Int) async {
        ...
        for shift in shifts.prefix(NotificationLimits.nextShiftHorizonDays) {
            ...
            guard let shiftDate = ShiftTechoCalendar.date(fromDayKey: shift.dayKey),
