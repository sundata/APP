...
class NewsService: ObservableObject {
    ...
    var autoRefreshCategory: NewsCategory?
    ...
    // MARK: - 获取新闻列表
    func fetchNews(category: NewsCategory? = nil, page: Int = 1, limit: Int = 40) async {
        ...
        do {
            ...
            await MainActor.run {
                ...
                // 异步为首屏无图文章轻量补图，避免拖慢列表加载
                if page == 1 {
                    self.fetchMissingImages(limit: 8)
                }
            }
        } catch {
            await MainActor.run {
                ...
                self.isLoading = false
                // ネットワークエラーの場合、既存のキャッシュデータを保持
                // キャッシュもない場合のみモック データを使用
                if self.articles.isEmpty && self.lastUpdated == nil {
                    loadMockData()
                }
            }
        }
        ...
        do {
            ...
            if subKeywords.count > 1 {
                for kw in subKeywords {
                    ...
                    let (retryData, _) = try await URLSession.shared.data(from: retryComponents.url!)
    ...
    // MARK: - カテゴリで検索
    func searchByCategory(_ category: NewsCategory) async -> [NewsArticle] {
        do {
            var components = URLComponents(string: "\(baseURL)/articles")!
            components.queryItems = [
    ...
    func fetchAllLatest(limit: Int = 50) async -> [NewsArticle] {
        do {
            var components = URLComponents(string: "\(baseURL)/articles")!
            components.queryItems = [
    ...
    private func startAutoRefresh() {
        Timer.scheduledTimer(withTimeInterval: 60, repeats: true) { [weak self] _ in
            Task { @MainActor in
                ...
                let isEnabled = defaults.object(forKey: "autoRefreshEnabled") as? Bool ?? true
                guard isEnabled else { return }

                let storedInterval = defaults.integer(forKey: "refreshInterval")
                let intervalMinutes = storedInterval == 0 ? 30 : storedInterval
                guard Date().timeIntervalSince(self.lastAutoRefresh) >= TimeInterval(intervalMinutes * 60) else {
                    return
                }

                self.lastAutoRefresh = Date()
                await self.fetchNews(category: self.autoRefreshCategory, limit: 30)
            }
        }
    }
    
    // MARK: - 为无图文章抓取OG图片（客户端抓取，避免Cloud Run IP被封锁）
    private func fetchMissingImages(limit: Int = 8) {
...
enum NewsError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .aiAnalysisLimitReached: return "本日のAI分析は終了しました"
        case .proFeatureRequired: return "この機能はProプラン限定です"
        }
    }
}
