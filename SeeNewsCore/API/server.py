...
RSS_FETCH_CONCURRENCY = int(os.environ.get("RSS_FETCH_CONCURRENCY", "8"))
AI_PREWARM_LIMIT = int(os.environ.get("AI_PREWARM_LIMIT", "8"))
...
class ArticleORM(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_articles_active_category_published", "is_active", "category", "published_at"),
    )
...
def ensure_db_indexes():
    """Create query-critical indexes for existing production databases."""
    index_statements = [
        """
        CREATE INDEX IF NOT EXISTS ix_articles_active_published
        ON articles (is_active, published_at DESC)
        """,
        """
        CREATE INDEX IF NOT EXISTS ix_articles_active_category_published
        ON articles (is_active, category, published_at DESC)
        """,
        """
        CREATE INDEX IF NOT EXISTS ix_article_ai_analysis_article_id
        ON article_ai_analysis (article_id)
        """,
    ]
    try:
        ...
        logging.getLogger("furinnews").warning(f"Failed to ensure DB indexes: {exc}")
...
def _contains_keyword(text: str, source_name: str, source_url: str = "") -> bool:
    ...
    if source_url in GOOGLE_TOP_URLS or source_url.startswith("https://news.google.com/rss?"):
        return True
...
def _infer_category(source_category: str, text: str) -> str:
    if source_category == "general" and any(kw in text for kw in POLITICAL_KEYWORDS):
        return "politician"
...
async def fetch_rss(source: dict) -> list[dict]:
    ...
    try:
        ...
        for entry in feed.entries:
            ...
            if not _contains_keyword(article_text, src_name, source["url"]):
                continue
            ...
            results.append({
                ...
                "category":     _infer_category(source["category"], article_text),
...
async def run_crawler():
    ...
    if _crawler_running:
        logger.info("crawler already running, skipping this tick")
        return

    _crawler_running = True
    logger.info("=== crawler started ===")
    try:
        sem = asyncio.Semaphore(RSS_FETCH_CONCURRENCY)

        async def fetch_with_limit(src: dict):
            async with sem:
                return await fetch_rss(src)

        tasks = [fetch_with_limit(src) for src in RSS_SOURCES]
        ...
        try:
            for a in all_articles:
                if not db.query(ArticleORM).filter_by(uid=a["uid"]).first():
                    ...
                    db.add(article)
                    new_articles_for_analysis.append(article)
                    new_count += 1
            db.commit()
            new_articles_for_analysis = [
                (article.id, article.title, article.summary)
                for article in new_articles_for_analysis
            ]
        ...
        # Pre-warm real AI analyses for the newest articles so the app can show results quickly.
        if openai_client:
...
def get_articles(
    ...
    try:
        ...
        total = ((page - 1) * limit) + len(rows) + (1 if has_more else 0)
        return NewsResponse(
...
async def ai_analyze(
    ...
    if article_id:
        ...
        try:
            ...
            # Always return real cached analysis immediately when available.
            cached = db.query(ArticleAIAnalysisORM).filter(
    ...
    try:
        result = await asyncio.wait_for(_analyze_with_openai(prompt), timeout=3.0)
    except asyncio.TimeoutError:
        result = None
