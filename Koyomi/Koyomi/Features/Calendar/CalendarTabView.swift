...
struct CalendarTabView: View {
    ...
    private var weekdayHeader: some View {
        HStack(spacing: 2) {
            ForEach(1...7, id: \.self) { weekday in
                Text(KoyomiCalendar.weekdaySymbol(weekday: weekday))
        ...
        return Group {
            if used.isEmpty {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    ...
                    .padding(.vertical, KoyomiTheme.Spacing.xs)
                }
            }
        }
    }
    ...
    private func weekdayColor(_ weekday: Int) -> Color {
        switch weekday {
        case 1: KoyomiTheme.sundayText(colorScheme)
        case 7: KoyomiTheme.saturdayText(colorScheme)
        default: KoyomiTheme.primaryText(colorScheme)
        }
    }
...
struct CalendarDayCell: View {
    ...
    var body: some View {
        VStack(spacing: 2) {
            HStack(spacing: 2) {
                Text("\(day.day ?? 0)")
                    .font(KoyomiTheme.captionFont.weight(isToday ? .bold : .regular))
                    .foregroundStyle(numberColor)
                if holidayName != nil {
                    Image(systemName: "flag.fill")
                        .font(.system(size: 7))
                        .foregroundStyle(KoyomiTheme.sundayText(colorScheme))
                }
            }
            if let assignment {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .stroke(isToday ? KoyomiTheme.accent : KoyomiTheme.cardStroke(colorScheme), lineWidth: isToday ? 2 : 1)
        )
    ...
    private var numberColor: Color {
        if holidayName != nil || day.isSunday { return KoyomiTheme.sundayText(colorScheme) }
        if day.isSaturday { return KoyomiTheme.saturdayText(colorScheme) }
        return KoyomiTheme.primaryText(colorScheme)
    }
