...
struct DataManagementView: View {
    ...
    private func handleImportSelection(_ result: Result<URL, Error>) {
        ...
        case .success(let url):
            do {
                ...
                errorMessage = "このファイルの形式（バージョン \(version)）には対応していません。新しいバージョンの Koyomi でお試しください。"
            } catch {
                errorMessage = "ファイルを読み込めませんでした。Koyomi で書き出した JSON を選んでください。"
            }
        case .failure:
    ...
    private func performImport(strategy: KoyomiStore.ImportStrategy) {
