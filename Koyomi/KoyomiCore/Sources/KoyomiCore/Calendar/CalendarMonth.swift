...
public struct CalendarMonth: Sendable {
    ...
    public init(year: Int, month: Int, calendar: Calendar = KoyomiCalendar.japan) {
        self.year = year
        self.month = month
    ...
    /// 指定日が含まれる月。
    public static func containing(_ date: Date, calendar: Calendar = KoyomiCalendar.japan) -> CalendarMonth {
        let components = calendar.dateComponents([.year, .month], from: date)
        return CalendarMonth(year: components.year ?? 1970, month: components.month ?? 1, calendar: calendar)
    }

    /// 月送り。
    public func adding(months: Int, calendar: Calendar = KoyomiCalendar.japan) -> CalendarMonth {
        var components = DateComponents()
        components.year = year
        components.month = month
        components.day = 1
