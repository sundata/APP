...
public enum JapaneseHolidayCalendar {
    ...
    public static func holidays(year: Int) -> [String: String] {
        ...
        for ordinal in statutory.keys.sorted() {
            ...
            guard statutory[candidate] == nil,
                  ...
                  KoyomiCalendar.weekday(for: date) != 1 else { continue }
        ...
        for (ordinal, name) in result {
            ...
            byDayKey[KoyomiCalendar.dayKey(for: date)] = name
        }
        return byDayKey
    }
    ...
    public static func holidayName(dayKey: String) -> String? {
        guard let parsed = KoyomiCalendar.components(fromDayKey: dayKey) else { return nil }
    ...
    private static func nthWeekday(year: Int, month: Int, weekday: Int, nth: Int) -> Int {
        ...
        components.day = 1
        guard let first = KoyomiCalendar.japan.date(from: components) else { return 1 }
        let firstWeekday = KoyomiCalendar.weekday(for: first)
    ...
    private static func ordinal(year: Int, month: Int, day: Int) -> Int? {
        ...
        guard let date = KoyomiCalendar.date(fromDayKey: KoyomiCalendar.dayKey(year: year, month: month, day: day), calendar: calendar) else {
            return nil
        }
